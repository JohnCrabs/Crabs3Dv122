#ifndef PARAMETER_C3D_H
#define PARAMETER_C3D_H
...
#define MAX_GROUP_SIZE 127
#define MAX_GROUP_PARAMETER_SIZE 127 
...
public:
    ...
    inline short int ParameterBlock(void) {return parameterBlock;} /* Return parameterBlock */
    inline short int Number_ID(void) {return idNumber;} /* Return idNumber */
    inline short int ParameterSize(void) {return numberOfParameters;} /* Return numberOfParameters */
    inline short int Processor(void) {return processorType;} /* Return processorType */

    /* Read the Parameter Header Block */
    Parameter_Header_c3d ReadParameterHeaderBlock(FILE* file);

    /*
     * Overloaded function which prints the parameter header to file.
     * Takes as parameter the address FILE* .
     */
    bool PrintParameterHeaderToFile(FILE* file);

    /*
     * Overloaded function which prints the parameter header to file.
     * Takes as parameter the path of the file.
     */
    bool PrintParameterHeaderToFile(std::string path);

private:
    short int parameterBlock; /* The block number of the first block in the parameter section. */
    short int idNumber; /* Key identifier of 80 decimal. */
    short int numberOfParameters; /* Number of parameter blocks to follow. */
    short int processorType; /*
...
class Group_Parameter_c3d
{
public:
    Group_Parameter_c3d(){}

    inline short int NameSize(void) {return characterNumber;} /* Return characterNumber */
    inline bool IsLocked(void) {return locked;} /* Return locked */
    inline short int ID(void) {return parameterID;} /* Return parameterID */
    inline std::string Name(void) {return parameterName;} /* Return parameterName */
    inline short int NextStart(void) {return nextParameterStart;} /* Return nextParameterStart */

    inline short int Format(void) {return byteFormat;} /* Return byteFormat */
    inline short int DimensionSize(void) {return numberOfDimensions;} /* Return numberOfDimensions */
    inline short int Dimension(const int index) {return parameterDimensions[index];} /* Return parameterDimensions[] */
    inline std::string ParameterChar(void) {return parameterDataChar;} /* Return parameterDataChar */
    inline short int ParameterByte(const int index) {return parameterDataByte[index];} /* Return parameterDataByte */
    inline short int ParameterInt_16(const int index) {return parameterDataInt_16[index];} /* Return parameterDataInt_16 */
    inline float ParameterReal(const int index) {return parameterDataReal[index];} /* Return parameterDataReal */

    inline short int DescriptionSize(void) {return descriptionSize;} /* Return descriptionSize */
    inline std::string Description(void) {return description;} /* Return description */

    bool ReadGroupParameter(FILE* file, Group_Parameter_c3d& parameter, const int flag);

    ~Group_Parameter_c3d(){}

private:
    short int characterNumber; /*
                                * Number of characters in Group name (1-127).
                                * This value may be set  to a negative number
                                * to indicate that the group is "locked".
                                */
    bool locked;

    short int parameterID; /*
                            * Parameter ID number (+1 to +127... always positive).
                            */

    char* parameterName; /*
                                * Parameter Name
                                * (ASCII characters-upper case A-Z, 0-9
                                * and underscore _ only)
                                */

    short int nextParameterStart; /*
                                   * A signed integer offset in bytes pointing to
                                   * the next group/parameter.
                                   */

    short int byteFormat; /*
    ...
    short int numberOfDimensions; /*
                                   * Number of dimensions (0-7) of the
                                   * parameter - 0 if the parameter is scalar.
                                   */

    short int parameterDimensions[MAX_DIMENSIONS]; /* Dimensions */

    /* Parameter Data */
    char* parameterDataChar; /* Character */
    short int* parameterDataByte; /* Byte */
    short int* parameterDataInt_16; /* 16-bit integer */
    float* parameterDataReal; /* Real, Floating-point */

    short int descriptionSize; /* Number of characters in the description */
...
public:
    ...
    inline short int NameSize(void) {return characterNumber;} /* Return characterNumber */
    inline bool IsLocked(void) {return locked;} /* Return locked */
    inline short int ID(void) {return groupID;} /* Return groupID */
    inline std::string Name(void) {return groupName;} /* Return grouprName */
    inline short int NextStart(void) {return nextGroupStart;} /* Return nextGroupStart */
    inline short int DescriptionSize(void) {return descriptionSize;} /* Return descriptionSize */
    inline std::string Description(void) {return description;} /* Return description */

    inline short int ParameterSize(void) {return parameterSize;} /* Return parameterSize */
    inline Group_Parameter_c3d Parameter(const int index) {
     ...
     *
     */
    bool ReadGroup(FILE* file, Group_c3d& group, const int flag, const bool isGroup);

    void CleanMemory(Group_c3d &group) { free(group.parameter); }
...
private:
    short int characterNumber; /*
                                ...
                                */
    bool locked;
    short int groupID; /* Group ID number (-1 to -127... always negative). */
    ...
    short int parameterSize; /* The real size of parameters */
    Group_Parameter_c3d* parameter;
...
public:
    ...
    inline Parameter_Header_c3d Header(void) {return header;}

    inline short int GroupSize(void) {return groupSize;} /* Return groupSize */
    inline Group_c3d Group(const int index) {
        if(index > 127 || index < 0)
            return group[0];
        else
            return group[index];
    ...
    Parameter_c3d ReadParameterHeaderBlock(FILE* file);
    Parameter_c3d ReadGroupParameterBlock(FILE* file, Parameter_c3d parameter, const int flag);

    bool PrintToFile(std::string path);

    void CleanMemory(void) {
        for(int i = 0; i != groupSize; ++i)
            group->CleanMemory(group[i]);
        free(group);
    }
...
public:
    ...
    Trial SetTrial(Parameter_c3d parameter);
...
public:
    ...
    /* Set Subject Values */
    Subjects SetSubjects(Parameter_c3d parameter);

    /* Clean Memory */
    void CleanMemory(Subjects& subjects);
...
public:
    ...
    /* Set Point Values */
    Point SetPoint(Parameter_c3d parameter);

    float MultiplierForMeters(Point point);

    void CleanMemory(Point& point);
...
private:
     ...
     * trajectories that match strings in POINT:ANGLES should be treated as three-dimensional
...
private:
     ...
     * channel scaling factors and acts on all channels. Common values for GEN_SCALE are:
     *
     ...
     * 0.0048828 to obtain analog data scaled in Volts when sampled by a 12-bit
     * ADC that is measuring a ± 10V input range, or 0.0024414 when measuring
     * with a ±5 V ADC input range.
     *
     ...
     * contains data with a POINT:RATE of 60.00 has 10 analog samples per 3D sample ( 60 x 10 ).
     *
...
public:
    ...
    Manufacturer SetManufacturer(Parameter_c3d parameter);

    void CleanMemory(Manufacturer& manufacturer);
